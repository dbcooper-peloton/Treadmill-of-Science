...
def count_breaks(df):
    """Count the number of breaks (marker rows) in the DataFrame."""
...
def remove_small_subsets(subset_dfs, min_size=100000):
    """Remove subset dataframes below a certain size."""
    subset_dfs_filtered = [subset_df for subset_df in subset_dfs if len(subset_df) >= min_size]
    return subset_dfs_filtered


def count_subset_lengths(subset_dfs):
    """Count the length of each subset dataframe."""
    subset_lengths = [len(subset_df) for subset_df in subset_dfs]
    return subset_lengths

# Which footstep do you want to plot
footstep = 0

# Replace 'your_file.xlsx' with the path to your Excel file
file_path = r'C:\Users\cooper\Documents\EE_STM\Advance_Running_Metrics\Emily\0Inc_0.8MPH.csv'
save_path = r'C:\Users\cooper\Documents\EE_STM\Advance_Running_Metrics\Emily\test.csv'
...
# Call the function to remove small subsets
subset_dfs = remove_small_subsets(subset_dfs)

# Add breaks after removing small subsets
subset_dfs_with_breaks = []
for subset_df in subset_dfs:
    subset_dfs_with_breaks.append(subset_df)
    marker_row = pd.DataFrame([['', '']], columns=["TIME", "VOLTAGE"])
    subset_dfs_with_breaks.append(marker_row)

# Concatenate the subset dataframes and marker rows
result_df = pd.concat(subset_dfs_with_breaks, ignore_index=True)
...
# Count how many breaks there are in the concatenated dataframe
num_breaks = count_breaks(result_df)
print("Number of breaks:", num_breaks)

# Plot a single footstep
plot_subset_by_break(result_df, footstep)
...
# Count the lengths of each subset dataframe
# subset_lengths = count_subset_lengths(subset_dfs)
# print("Lengths of each subset dataframe:", subset_lengths)
