...
//
// If pBuffer is 0, the length of the required buffer (in wchar_t character counts) is returned in bufferSize. 
// This count includes space for a null terminator character.  (Byte size of the buffer should be at least 2 x charCount).
...
// Constructions a sensor configuration using all found sensors. Default calibration pressure ranges are utilized.
// This function sets the pressure units to ePRESUNIT_MMHG! Be sure to call XS_SetPressureUnit() after calling this function.
// 
// The XSN variant function takes a full file path and filename to construct an XSENSOR session file from all recorded frames.
//
// Example:  
...
// Sets the calibrated data's pressure units (see enum EPressureUnits). Pressure readings are presented in these units. 
// Returns xTRUE if the call succeeds.
...
// Adds a sensor to the sensor configuration. Will download any calibration files from the sensor if they are not present.
...
//
// NOTE: When streaming mode is on, the DLL will allocate large amounts of buffer memory if the samples are not
// consumed fast enough.
//
// The streamingMaxFrameCache parameter determines how many frames the streaming mode buffers. This is the cache limit.
...
//
// When using the library in an un-threaded mode, this call blocks until a sample is recorded or
...
//
// pData 
//	- pass in a pre-allocated buffer which is larger enough to hold (rows X columns) elements, each 32-bits.
...
// Retrieves the hardware frame header state. 'sequence' increments for each frame. 'ticks' is milliseconds for X4
...
// Converts a wide string (wchar_t) to UTF8. You must supply a UTF8 buffer that is large enough to hold the converted string.
...
//  - If utf8 is not null, pass in the utf8's buffer size in this field to allow for an extra buffer safety check.
// 
// char* utf8 
//	- the buffer to hold the utf8 string. Pass in nullptr (or 0) to retrieve just the size of the buffer (in bytes) required in bufferSize.
...
// 
// uint32_t& bufferSize 
//	- returns the size of the buffer (in byte count) needed to hold the conversion. wchar_t uses 2 bytes per character.
//  -  (eg: If bufferSize == 100, then  wchar_t wide[50]  will hold the string.
//  - If wide is not null, pass in the wide's buffer size (in bytes) in this field to allow for an extra buffer safety check.
// 
// wchar_t* wide 
//	- the buffer to hold the wide string. Pass in nullptr (or 0) to retrieve just the size of the buffer (in bytes) required in bufferSize.
...
#if(0) // work in progress - 2022-Aug-17 - T.RUSSELL - XSENSOR TECH.
...
extern "C" XSCORE_API XBOOL X4_PrepForRemoteRecording(SENSORPID spid, float framesPerSecond);
// Uploads state needed for remote recording. Can take 20 - 60 seconds.
// This allows the X4 start record commands to run immediately ... otherwise they must perform the same prep.

extern "C" XSCORE_API XBOOL X4_StartRemoteRecording(SENSORPID spid, float framesPerSecond);
// Starts the X4's remote recording.
// enableIMU - enables IMU recording
